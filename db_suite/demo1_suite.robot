...
TC3
    Execute Sql String    Insert into Products (product_id,productname,description) values (655,'Nav','Hi')
    Row Count Is Equal To X    select * from Products where product_id=655    1
...
TC5
    Execute Sql String    update Products set productname='Nav' where product_id=655
    @{output}    Query     select * from Products where product_id=655
    Log To Console    ${output}
